...
def _strip_moe_dispatch_from_partition(partition):
    ...
    An OverlapOp that contains no dispatch is returned as-is rather than
    rebuilt: identity is cheap to keep, and the sweep walks this path once per
    candidate per layer.
    """
    from aiconfigurator.sdk import operations

    def _same_ops(new, old):
        return len(new) == len(old) and all(a is b for a, b in zip(new, old, strict=True))

    def rewrite(op):
        if not isinstance(op, operations.OverlapOp):
            return None if _is_moe_dispatch_op(op) else op
        group_a = [o for o in (rewrite(i) for i in op._group_a) if o is not None]
        group_b = [o for o in (rewrite(i) for i in op._group_b) if o is not None]
        if not group_a and not group_b:
            return None
        if _same_ops(group_a, op._group_a) and _same_ops(group_b, op._group_b):
            return op
